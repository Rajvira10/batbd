...
@section('custom-script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var dTable = $('#memberTable').DataTable({
                order: [],
                lengthMenu: [
                    [10, 25, 50, 100, -1],
                    [10, 25, 50, 100, "All"]
        ...
        const approveMember = (id) => {
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to approve this member!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('admin.members.approve') }}",
                        method: 'POST',
                        data: {
                            member_id: id,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                toaster('Member Approved Successfully', 'success');
                            } else {
        ...
        const deleteMember = (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('admin.members.destroy') }}",
                        method: 'POST',
                        data: {
                            member_id: id,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(response) {
                            $('#memberTable').DataTable().ajax.reload();
                            if (response.success) {
                                toaster('Member Deleted Successfully', 'success');
                            } else {
                                toaster('Something went wrong', 'danger');
                            }
                        },
